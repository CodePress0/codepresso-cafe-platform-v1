...
public class PasswordFindService {
    ...
    private final SimpleEmailAuthService emailAuthService;
    ...
    public PasswordFindResponse findPassword(PasswordFindRequest request) {
        ...
        try {
            // 이메일 발송 시도
            String verificationCode = emailAuthService.generateAuthCode(request.getEmail());
    ...
    public PasswordResetResponse resetPassword(PasswordResetRequest request) {
        ...
        // 메모리 맵에서 인증번호 검증
        if (!emailAuthService.verifyAuthCode(request.getEmail(), request.getVerificationCode())) {
